...
class FileSys{
    ...
    ******************************************/
    int nextPos(File file);
    
    bool doInsert(bool checkO, File file);
    ...
    int getFileHelper(string name, int i, prob_t method, int size) const;

    bool removeHelper(File** &table, int size, File file, prob_t method);
    File findHelper(File** table, int size, string name, int block, prob_t method) const;

    void clear(File** &table, int size);
};
